...
def doubao_translate_text(text, target_lang='zh'):
    ...
    2. 若遇到敏感词汇（如涉及具体的 “台独” 分子名字、一些可能引起争议的特定政治事件细节等），在不影响对正确内容理解和表达的前提下，以恰当的方式进行脱敏处理；若无法直接脱敏，需在翻译后用""将相关内容括起来并附上正确的解释说明以消除错误影响；
    3. 对于表述模糊或有歧义的地方，依据客观事实、合理逻辑和中国相关的法律、政策以及国际社会公认的准则等背景知识进行清晰、明确且准确的翻译；
    4. 翻译完成后，在译文后单独列出对原文中错误内容的纠正说明，包括错误点、正确的事实或观点等，以便清晰地呈现对错误内容的处理过程和结果，该部分用“【”和“】”包起来。
    # 需要翻译的内容
    {text}"""},
                ],
            )
